...
const EditOperationScreen = ({navigation, route}) => {
  ...
  const loadData = async () => {
    try {
      const [groupsData, shiftsData, linesData, operationData] =
        await Promise.all([
          getGroups(),
          getShifts(),
          getProductionLines(),
      ...
      setDate(new Date(operation.operationDate));
      setGroupId(operation.groupId.toString());
      setShiftId(operation.shiftId.toString());
      setProductionLineId(operation.productionLineId.toString());
      setTemperature(operation.temperature.toString());
      setWeight(operation.weight.toString());
      setQuality(operation.quality);
      setInputMethod(operation.inputMethod);
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Sukses', 'Operasi berhasil diupdate', [
        {text: 'OK', onPress: () => navigation.goBack()},
      ]);
    } catch (error) {
      Alert.alert(
        'Error',
        error.response?.data?.message || 'Gagal mengupdate operasi',
      );
    } finally {
      setSubmitting(false);
    }
...
export default EditOperationScreen;
